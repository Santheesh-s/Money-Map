...
const TransactionHistory = () => {
  ...
  // Build and download an Excel (.xlsx) of currently filtered transactions
  const handleDownloadExcel = () => {
    const data = filtered.map(t => ({
      Date: new Date(t.date).toLocaleDateString(),
      Type: t.type,
      Category: t.category || '',
      Amount: (t.type === 'income' ? '+' : '-') + ' ' + (t.currency || '₹') + Math.abs(t.amount),
      Status: t.status || '',
    ...
    XLSX.writeFile(workbook, `transactions_${y}-${m}-${d}.xlsx`);
  };
  ...
  return (
    <div className="history-container">
      ...
      <div className="filters">
        ...
        <button onClick={handleDownloadExcel} title="Download Excel" className="btn btn-primary">
      ...
      <div className="pagination">
        ...
        {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
          <button className="btn btn-ghost" key={p} onClick={() => setPage(p)} style={{ fontWeight: p === page ? 'bold' : 'normal' }}>{p}</button>
        ))}
