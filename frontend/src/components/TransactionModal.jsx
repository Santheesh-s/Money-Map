...
import { checkBudgetLimits, showBudgetWarnings } from '../utils/budgetNotifications';
...
const TransactionModal = ({ isOpen, onClose, initialData, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Check budget limits before saving
    if (formData.type === 'expense') {
      const warnings = await checkBudgetLimits(payload);
      if (warnings.length > 0) {
        showBudgetWarnings(warnings);
        // Ask for confirmation if there are error-level warnings
        const hasErrors = warnings.some(w => w.type === 'error');
        if (hasErrors) {
          const confirmed = window.confirm(
            'This transaction will exceed your budget limits. Do you want to continue?'
          );
          if (!confirmed) return;
        }
      }
