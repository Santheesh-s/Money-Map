...
const { checkBudgetThresholds } = require('../services/budgetNotificationService');
...
router.post('/', async (req, res) => {
  try {
    ...
    // Check budget thresholds after creating transaction
    if (saved.amount < 0) { // Only check for expenses
      checkBudgetThresholds(req.userId, saved._id).catch(err => 
